...
class RFMRtlKhGDZY():
    ...
    def GdzyRfm(self):
        ...
        #1.1 获取省份、地市
        city_list = dataGatherBase().get_city_code(self.Param.sql)
        # print(city_list)
        # print(self.Param.sql)
        # 1.2 提取需要统计的规格：一、二类
        # self.logger.info('获取规格...')
        ...
        for key,row in city_list.iterrows():
            self.logger.info("处理的中烟为："+ str(RfmParam.corp_name) + "；处理的时间为：" + str(self.Param.busidate)+\
            ...
            # print(all_data)
            # 2.1.1 获取档位信息
            self.logger.info('获取档位信息...')
            cust_level = dataGatherBase().get_cust_level(row['CITY_NAME'].rsplit('市',1)[0])

            # 2.1.2 合并档位
            self.logger.info('正在合并档位...')
            all_data = pd.merge(all_data,cust_level,how='left',on='CUST_CODE')

            # 2.2 中位数判断高低并插入数据库
            logger.get_logger().info('开始计算零售户的分类...')
            # max_week_id = weeks_month[['week_id']].max()['week_id']
            self.func.get_rtl_class(all_data, 'max_week_id', self.Param.mysql_tab, self.Param.mysql_tab_list)
