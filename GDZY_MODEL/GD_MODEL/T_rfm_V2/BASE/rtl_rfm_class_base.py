...
class RtlRfmMedianClass():
    ...
    def get_rtl_class(self,allorderdata,current_week,tab_name,colist,value_w=[1/3,1/3,1/3],type_of_class='50%'):
        ...
        if len(allorderdata)==0:
            ...
            # #计算分类
            allorderdata[['R_NUM']]=allorderdata[['R_NUM']].astype('float64')
            statistic    = allorderdata['R_NUM'].describe()

            value = statistic.loc[type_of_class]
            self.logger.info('R阈值为：%f'%value)
            if value==0:
                WetherBiggerThan = allorderdata['R_NUM'] <= value
            else:
                WetherBiggerThan = allorderdata['R_NUM'] < value
            WetherBiggerThan = WetherBiggerThan + 0

            allorderdata['R_CLUSTER'] = WetherBiggerThan
            # print(allorderdata.loc[:, ['R_NUM','R_C', 'F_CLUSTER', 'M_CLUSTER']])
            new = allorderdata[['F_CLUSTER','M_CLUSTER']].replace('低',0)
            new = new[['F_CLUSTER', 'M_CLUSTER']].replace('高', 1)
            allorderdata[['F_CLUSTER', 'M_CLUSTER']] = new
            # print(allorderdata[['F_CLUSTER', 'M_CLUSTER']])
            # 总的分类结果
            for index in range(len(allorderdata)):
                allorderdata.loc[index, 'CUST_CLUSTER_CODE'] = ''.join([str(i) for i in allorderdata.loc[index,['R_CLUSTER','F_CLUSTER','M_CLUSTER']]])

            new1 = allorderdata[['R_CLUSTER','F_CLUSTER', 'M_CLUSTER']].replace(0, '低')
            new1 = new1[['R_CLUSTER','F_CLUSTER', 'M_CLUSTER']].replace(1, '高')

            allorderdata['R_CLUSTER'] = new1['R_CLUSTER']
            allorderdata['F_CLUSTER'] = new1['F_CLUSTER']
            allorderdata['M_CLUSTER'] = new1['M_CLUSTER']
