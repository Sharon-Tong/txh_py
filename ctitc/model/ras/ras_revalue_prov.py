...
#########################################################
class RASRevalueProv(RASBase):
    ##########################################
    # 因子分析模型初始化
    ##########################################
    def __init__(self):
        super().__init__(logkey="ras_rev")
    pass
    ...
    def main(self):
        ...
        try:
            db.connect()
            # 1.查询需要处理的规格及规格组别代码
            s_total = "select BAR_CODE,BAR_NAME, GROUP_NAME, GROUP_CODE from RAS_BAR_CONFIG_PROV " \
                      " where IS_USE = '1' order by  GROUP_CODE, BAR_CODE "
    ...
    def getStdData(self, bars=None):
        ...
        #     data = pd.DataFrame(list(self.cur_rec))
        #     data.fillna(0.0, inplace=True)
        # finally:
        #     db.close()
        # pass
        rec, data = self.getProvInfoFromHive(sql_condition, '1')
        self.cur_rec = rec
    ...
    def doPCA(self, C, ratio, grp=None, grp_nm=None, pro_code='0'):
        ...
        # print("\ngrp_code:\n", grp_code)
        pca_t_score, fa_t_score = self.getScore(self.X_std,C, ratio,grp_code, grp_name, 'RAS_LOADING_HEALTH_MONTH_PROV')
        ...
        try:
            ...
            # 2.插入主成分综合得分表
            # 2.1 插入前先删除主成分综合得分表
            d_score_sql = "delete from RAS_SCORE_HEALTH_MONTH_PROV where GROUP_CODE='" + grp_code + "' " \
                               " and PRO_CODE='" + pro_code + "'"
            db.delete(d_score_sql)
            count = 0
            PRE_SQL = "INSERT INTO RAS_SCORE_HEALTH_MONTH_PROV (GROUP_CODE, BUSI_DATE, PRO_CODE, PROV_CODE, PROV_NAME, " \
                      " BAR_CODE, BAR_NAME,  PCA_SCORE, FACTOR_SCORE, STD_PCA_SCORE, STD_FACTOR_SCORE) VALUES "
            print(len(self.cur_rec))
            print(len(fa_t_score))
            for row in self.cur_rec:
                bar_code = row[2]
                bar_name = row[3]
    ...
    def simulateScore(self, condition=None,fields=None):
        ...
        try:
            ...
            s_total = "select BAR_CODE,BAR_NAME, GROUP_NAME, GROUP_CODE from RAS_BAR_CONFIG " \
                      " where IS_USE = '1' and GROUP_CODE = '" + con_grp_code + "' order by  BAR_CODE "
            barcodes = db.select(s_total)
        finally:
            db.close()
        pass
