...
#读取原数据
csvfile = r"C:\Users\Tangt\Desktop/价格处理CSV 删空格(9).csv"   #文件存放路径
data_org = data = pd.read_csv(csvfile,header=0, engine="python")
n=len(data_org)
print("一共需要处理的数据为："+ str(n))


##删除重复行
new_data=data_org.drop_duplicates(inplace=False)
data_org=new_data
print("一共删除处理的数据为："+ str(n-len(new_data)))
#涉及的城市
...
new_sql=PRE_SQL+i_sql


open( r"C:\Users\Tangt\Desktop\insert.txt","wt").write(new_sql)
