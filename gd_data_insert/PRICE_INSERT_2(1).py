...
import datetime
import ibm_db;
from logingPrint.logp1 import Logger

Start_time = datetime.datetime.now()

#读取原数据
csvfile = "/Users/yaochunling/Desktop/价格处理CSV 删空格.csv"   #文件存放路径
data_org = pd.read_csv(csvfile,header=0, encoding="GB18030")
data_org.drop_duplicates(subset=['BAR_CODE','CIG_TRADEMARK','MONTH_ID','DATE_ID','SAMPLE_CODE','PRICE_PLACE_NAME','CIRCL_PRICE'], inplace=True)
...
#print (type(data_existed.loc[0,'DATE_ID']))
#print (data_existed)
...
for row in data_org.index:
    ...
    DATE_ID          = datetime.date(*map(int, str(insert_data[3]).split('-')))
    ...
    #检查这句话是否已经存在：
    '''
    data_existed1 = data_existed[data_existed['BAR_CODE']==bar_code]
    print (data_existed1)
    print (DATE_ID)
    data_existed2 = data_existed1[data_existed1['DATE_ID']==DATE_ID]
    print (data_existed2)
    data_existed3 = data_existed2[data_existed2['SAMPLE_CODE']==SAMPLE_CODE]
    print (data_existed3)
    if len(data_existed3) > 0:
        pass
    '''
